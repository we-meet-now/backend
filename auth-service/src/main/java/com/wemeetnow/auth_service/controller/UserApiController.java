...
public class UserApiController {
    ...
    @GetMapping("/get-id")
    public ResponseEntity<CommonApiResponse<AuthUserDto>> getUserIdFromAccessToken(HttpServletRequest request) {
        String authorizationHeader = request.getHeader(HttpHeaders.AUTHORIZATION);
        String token = authorizationHeader.replace("Bearer ", "");

        if (JwtUtil.isExpired(token)) {
            AuthUserDto errorDto = AuthUserDto.builder().userId(0L).statusCd("4010").statusMsg("토큰이 만료되었습니다.").build();
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                    CommonApiResponse.<AuthUserDto>builder()
                            .statusCode("4010")
                            .data(errorDto)
                            .message("토큰이 만료되었습니다.")
                            .build()
        ...
        AuthUserDto responseDto = AuthUserDto.builder()
                .userId(userId)
                .statusCd("2000")
                .statusMsg("success")
                .build();

        return ResponseEntity.ok(CommonApiResponse.<AuthUserDto>builder()
                .statusCode("2000")
                .data(responseDto)
    ...
    public ResponseEntity<CommonApiResponse<ChatParticipantUserDto>> getUserInfoFromAccessToken(
        ...
        if (JwtUtil.isExpired(token)) {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
                    CommonApiResponse.<ChatParticipantUserDto>builder()
                            .statusCode("4010")
                            .data(null)
        ...
        return ResponseEntity.ok(CommonApiResponse.<ChatParticipantUserDto>builder()
                .statusCode("2000")
                .data(responseDto)
    ...
    public ResponseEntity<CommonApiResponse<ChatParticipantUserDto>> getUserById(@PathVariable("userId") Long userId) {
        User findUser = userService.getUserById(userId).orElse(null);
        if (findUser == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                    CommonApiResponse.<ChatParticipantUserDto>builder()
